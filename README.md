...
- `广告诊断`：原广告分析工作台。
- `投放关键词检查`：独立读取 Bulk 里的关键词、匹配类型、竞价、广告活动、广告组和否定词，标注缺精准、竞价倒挂、重复分散、正负冲突、搜索出单未承接等结构问题。
...
- 投放关键词检查：吸收课程中可验证的数学思想，把广告看成 `最大化订单` 且受预算、库存、目标 ACOS/CPS 约束的问题；精准词作为效率控制位，广泛/词组作为探索位，否定词用于流量切分。
...
- 投放关键词检查支持自动检查全部投放词，也支持手动粘贴关键词列表逐词检查，并导出当前筛选后的 CSV。

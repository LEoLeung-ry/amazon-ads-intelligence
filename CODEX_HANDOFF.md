...
- Campaign-level CPS overrides are allowed; empty overrides inherit the product default CPS.
...
- Because that mobile goal helper is hidden before import, the goal input labels themselves must carry the explanation: use `目标 CPS（每单上限）` and `自然 CVR（转化率）`, not bare labels such as `默认目标 CPS` / `自然 CVR`.
- Before Bulk is loaded, hide the campaign/activity search card. Search is a refinement tool after campaigns exist; showing it before upload creates a false first step and pushes the upload/workflow path down the page.
- Before Bulk is loaded, the upload card should read `先上传 Bulk 工作簿`, the browse button should read `选择 Bulk`, and the subtitle should say CSV/SciAds can be added later. After Bulk is recognized, it can switch back to supplemental-file language.
- On mobile before Bulk is loaded, keep the normal `.import-assist.info` upload checklist as compact chips (`必传 / 最好有 / 可选`) and hide long row descriptions. Do not apply this compact treatment to `.import-assist.warn`; wrong-file recovery needs the full explanation and buttons.
- Wrong-file recovery must use operator language, not parser language. If a CSV cannot be recognized, say that it cannot generate the ad diagnosis by itself and that Bulk xlsx/xls is the required main file; explain that CSV is only a time-of-day supplement when it has `开始时间 / 广告活动名称 / 7天总销售额`.
- If a CSV looks like a standalone search term report (`客户搜索词` / `Customer Search Term` plus campaign or metric headers), `parseFile()` should show a specific recovery message: search terms must come from the Bulk workbook's `商品推广搜索词报告` sheet so the system can join campaign, ad group, bids, negatives, and action rows. Do not silently accept standalone search term CSV as the main analysis input.
- On mobile before Bulk is loaded, show only the current active workflow step in `workflow-strip` and compact the `workflow-compass`. The full five-step path remains available on desktop, but mobile first-time users should see the Step 1 action card within the first viewport instead of scrolling past all five steps before they can act.
- On mobile before Bulk is loaded, hide the topbar `status-pills` because they repeat the active Step 1 state and push the workflow compass down. Keep the product goal inputs in two columns even under 560px so `目标 CPS（每单上限）` and `自然 CVR（转化率）` stay editable without adding another vertical block. In the normal `.import-assist.info` state, hide the `上传顺序` title and show only compact chips; do not apply this to `.import-assist.warn`. On ultra-narrow screens (`max-width: 340px`), hide the duplicate main workspace `.topbar`, keep only the upload card's primary `选择 Bulk` button, hide the single-step `workflow-strip`, and reduce `workflow-compass` vertical padding; the demo entry remains in the Step 1 workflow card.
- In the default `#emptyState` for 广告诊断, hide the duplicated `.hero-upload-actions` before Bulk is loaded. The left upload card and `workflow-compass` already provide upload/demo actions; a third CTA row makes the first screen feel repetitive. Do not apply this rule to `#keywordEmptyState`, which still needs its own upload buttons when users enter the keyword checker first.
- After Bulk is loaded on mobile, make the page action-first rather than setup-first. Hide post-import demo/field-list buttons, relabel the left upload button from `选择文件` to `补充文件`, show only the current workflow step, suppress repeated topbar/helper copy, and keep the sidebar goal card to editable CPS/CVR inputs. The action queue should begin within the first mobile viewport after real data loads; detailed derived ACOS/CPS interpretation remains available in the action queue goal checkpoint.
- On first real Bulk import on mobile (`max-width: 820px`), `handleFiles()` should scroll `#actionQueue` into view after `renderAll()`. This is intentional: the product already defaults to the narrowed `本轮执行包`, so a mobile operator should land on the work order instead of rereading upload/search/goal setup. The demo-data path already performs this scroll; keep real Bulk behavior aligned with demo behavior. Also hide the duplicate `.brand-block` after Bulk on mobile because the workspace title replaces it.
- Detail tabs are rendered by `renderDetailTabs()` with compact state metadata (`总览 / 活动数`, `标的诊断 / 标的数`, `搜索词 / 条数`, `分时 / 已导入或未导入`, `广告位 / 组数`, `规则库 / 内置或条数`). Keep this status-in-tab pattern instead of adding another explanatory guide block; the second-level navigation should help operators choose the right detail view without increasing reading load.
- Inside `.queue-table-card`, keep `renderLeadAction(filteredQueueRows)` directly under the table header and before batch review/export actions, column controls, review summaries, review guide, filters, mobile cards, and the full table. The batch review/export buttons are rendered as `queueExportActions` after `.lead-action`, and `renderColumnControls()` deliberately inserts the `productActionTable` column selector after that direct `.queue-export-actions` block. Column settings and batch confirmation are useful, but the operator should see the first concrete backend action (`#1`, action, evidence, backend path, review buttons) before configuration controls or generic confirmation guidance.
- On mobile, `.lead-action-mobile-review` duplicates the row review control immediately after the first action evidence, while `.lead-action-side .review-control` is hidden. This is deliberate: the first action card can be tall, so operators should be able to mark `待确认 / 已确认 / 暂缓` after reading the first action and evidence, without scrolling to the bottom of the card. Keep the evidence single-line on mobile to preserve this fast review path.
- When a mobile operator clicks a review status inside `.lead-action-mobile-review`, `handleActionQueueClick()` should keep the next `.queue-table-card .lead-action` visible by scrolling it to the center after `renderAnalysis()`. This preserves a fast one-by-one review loop: read evidence, mark status, immediately see the next action.
- The 2026-06-06 `71f330a` Pages deployment failed at the deploy step even though build/artifact upload succeeded. When pushing, verify both GitHub raw files and the Pages deployment status/served resource versions; if raw is updated but Pages still serves the previous `index.html`, do a small cache-version/handoff update to retrigger deployment rather than claiming the site is live.
...
- After Bulk/demo data is loaded, the keyword checker insight strip now starts with a full-width `结构下一步` lead card generated by `keywordLeadInsight(actionCounts)`. Its button uses `data-keyword-action-preset` to set `state.filters.action`, sync `#keywordActionFilter`, rerender the table, and scroll the operator to the filtered rows. Keep this as the first structure action prompt rather than adding another dense report block.
- The action queue export buttons are now review-first: before any rows are confirmed, `确认本轮` / `确认待处理` is the primary button and `导出当前视图` is secondary. `导出已确认` becomes the primary execution-export button only after rows are confirmed. Preserve this hierarchy so new operators do not skip review and treat an unconfirmed current-view snapshot as a backend execution package.
- On mobile, `.queue-export-actions button:disabled` is hidden instead of merely dimmed. Keep this mobile-only behavior: disabled revoke/export/confirm buttons are valid state indicators on desktop, but on narrow screens they create extra row height and distract from the one action the operator can take now.
- After actions are confirmed, keep only the `execution-coach` export button as the primary confirmed-export CTA across desktop and mobile. Hide duplicate confirmed-export buttons in the table header, execution package, and export receipt (`queue-confirmed-export`, `package-export-btn`, `receipt-export-btn`). The workflow compass should route the operator back to the execution package (`去导出执行包` / `回到执行包`) instead of creating another direct export button.
...
- Keep the workflow strip and workflow compass step numbers aligned. Once any rows are confirmed, the operator has finished Step 4 and the next instruction must read as Step 5 / export execution package, even if a current-view snapshot was already exported. Do not show `第 4 步` in the compass while the strip highlights `导出结果`.
...
- Do not make the default workflow campaign-by-campaign; the user specifically wants fewer cross-campaign jumps.
- Do not reintroduce a crowded first screen full of KPI/table content before import.
- Do not hide the reasoning behind recommendations, but keep formulas and raw evidence in expandable or secondary layers.
- Update this `CODEX_HANDOFF.md` on every GitHub push, even for small UI or documentation-only changes.
...
- It must show execution order, execution group, review status, action, backend execution action, backend path, object, campaign, ad group, next step, evidence, priority reason, recommended bid, and optional expert metrics.
- Default queue columns should be execution-first: `执行顺序`, `执行分组`, `确认`, `动作`, `后台动作`, `后台定位`, `对象`, `活动`, `广告组`, `下一步`, `证据`, `优先理由`, `建议竞价`. Keep `当前 CPS`, `目标 CPS`, `ACOS`, and `原始原因` available as expert/custom columns.
- `withExecutionOrder(rows)` annotates the current filtered/sorted queue scope with `executionOrder: index + 1`. Recompute this after filtering and sorting so on-screen order, mobile cards, `导出当前视图`, and `导出已确认` all agree.
- `withExecutionGroups(rows)` copies `executionPlanLabel(row)` into `executionGroup` for every visible/exported queue row. Keep `执行分组` in the default queue CSV so operators can group exported rows by backend action family.
...
- `reviewControl(row)` should display the same full status labels (`待确认`, `已确认`, `暂缓`) in the table and lead/mobile cards. The active status color communicates the current choice, but the text must remain explicit for new operators.
...
- In the default `今日优先` batch, the main review button is labeled `确认本轮`. In expanded candidate-pool scopes, it is labeled `确认待处理`. The unconfirmed snapshot export is always labeled `导出当前视图`.
- `导出当前视图` exports the currently filtered queue rows, not the unfiltered queue. `导出已确认` exports confirmed rows matching the current action/risk/impact filters, independent of the selected review-status filter. Both exports include the recalculated `执行顺序` column so the CSV can be used as an ordered execution sheet. Keep `productQueueConfirmed` wired to the same columns so current column choices apply to both exports.
- Before any row is confirmed, the visible current-view export button must be labeled `导出当前视图` and styled as secondary. It is a snapshot/review export, not the main execution path. The primary path is `确认本轮` / `确认待处理` -> `导出已确认`.
- Current-view snapshot export must not advance the workflow compass to Step 5. It can display an export receipt and filename, but its next action remains `回到本轮确认` when no rows are confirmed, or `导出已确认` when confirmed rows already exist.
...
- `搜索出单未承接`: search term has orders and acceptable structure-reference ACOS but no positive keyword coverage. Do not describe this as an ACOS steering target; product CPS remains the main target.
...
Lead insight priority:

- `renderKeywordInsights(actionCounts)` must render the `结构下一步` card first, before the three supporting insight cards.
- `keywordLeadInsight(actionCounts)` chooses the first available action in this order: `搜索出单未承接`, `正负冲突`, `缺精准`, `竞价倒挂`, `重复分散`, then `结构暂时稳定`.
- The lead card should explain one first action in operator language and provide a single filter button when an actionable issue exists. Do not turn it into a KPI grid or a formula tutorial; formulas belong in existing brief/table evidence layers.
- The click handler for `[data-keyword-action-preset]` is intentionally inside `bindKeywordEvents()` and only changes the keyword action filter. It must not mutate uploaded data, action counts, product CPS goals, or the main product queue.
...
- `#fieldChecklistBtn`, `#heroFieldChecklistBtn`, and import-assist buttons with `data-download-field-checklist` call `downloadFieldChecklist()`. This generates a local CSV field checklist for Bulk/hourly/SciAds inputs. It is not a sample ad report and must not contain real advertising data.
...
- `renderImportAssist()` must stay structured, not a single vague sentence. It renders title, body, lightweight recognition chips, a three-row checklist, and contextual recovery buttons for the current state: default upload order, wrong/unrecognized file, optional-file-loaded-but-Bulk-missing, or Bulk-loaded success.
- `renderImportReceipt()` adds compact chips such as `Bulk 待补`, `Bulk 440 活动 / 28456 搜索词`, `小时 N 行`, `SciAds N 条`, or `N 个未识别`. Keep this as a fast file-intake receipt so first-time users know what the browser recognized and what is still missing.
- Wrong/unrecognized-file and optional-file-loaded-but-Bulk-missing states must include direct recovery actions inside the assist card, not just explanatory copy. The current actions are choose Bulk, download the field checklist, and optionally load demo data.
...
- Standalone search term CSV recovery is intentionally guidance-only. The parser recognizes this common mistake and explains why Bulk is required, but it does not add a separate search-term-CSV import workflow.
...
5. Verify:
   ...
   - Row review controls use explicit labels `待确认`, `已确认`, `暂缓`; confirmed rows should visibly show `已确认` as the active status.
   ...
   - `今日优先` focus strip appears after Bulk import. With the real Bulk fixture the first post-import queue view should default to 30 focused rows and `确认本轮 (30)`, while `查看候选池` restores the full 5,927-row queue/export scope.
   ...
   - `导出当前视图` respects the active queue filters and current custom column setup, but remains a secondary snapshot export until rows are confirmed.
   - After clicking `导出当前视图` before confirmation, the workflow compass says the export is a current-view snapshot and still tells the operator to confirm actions. It must not say `已导出 ... 执行数据`.
   ...
   - After loading demo or Bulk data, the keyword insight strip starts with `结构下一步`; when a priority issue exists, clicking its `筛选...` button updates the action filter and table rows to that issue.
...
- After Bulk import, the left sidebar must keep the operator controls reachable in this order: campaign search, product goal, activity scope/campaign list, then import receipts and supplemental file upload. The sidebar uses vertical scrolling and must not use `overflow: hidden`; imported-file receipts are supporting context and should not push `产品目标` below the fold.
- Do not hide `.activity-controls` or `.campaign-panel` in the imported/narrow layout. Operators often upload the product Bulk first and only then decide which campaign to inspect or override; the activity picker is part of the main post-import workflow, not a secondary desktop-only control.
...
- Queue exports call `focusQueueExportReceipt()` after `renderAnalysis()`. Keep this behavior: downloading the CSV is not enough feedback, especially on mobile. After `导出已确认` or `导出当前视图`, the operator should immediately land on `.export-receipt`, see row/column counts, filename, and the backend execution next steps.
- Current-view snapshot receipts use `data-start-lead-review` for `继续确认动作`. This should call `scrollToLeadAction()` and land on the first `.lead-action`, not the non-interactive `.round-plan`; the button promises continued review, so the next visible target must be the concrete action card and its review controls.

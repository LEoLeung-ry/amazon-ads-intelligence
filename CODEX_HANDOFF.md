...
- On mobile after actions are confirmed, keep only the `execution-coach` export button as the primary confirmed-export CTA. Hide duplicate confirmed-export buttons in the table header, execution package, and export receipt (`queue-confirmed-export`, `package-export-btn`, `receipt-export-btn`) so the operator has one obvious next click instead of several identical `导出已确认` buttons.

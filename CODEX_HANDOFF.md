...
- Inside `.queue-table-card`, keep `renderLeadAction(filteredQueueRows)` directly under the table header/export actions and before column controls, review summaries, review guide, filters, mobile cards, and the full table. `renderColumnControls()` deliberately inserts the `productActionTable` column selector after `.lead-action`; column settings are useful, but the operator should see the first concrete backend action (`#1`, action, evidence, backend path, review buttons) before configuration controls or generic confirmation guidance.

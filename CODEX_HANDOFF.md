...
- On mobile before Bulk is loaded, hide the topbar `status-pills` because they repeat the active Step 1 state and push the workflow compass down. Keep the product goal inputs in two columns even under 560px so `目标 CPS（每单上限）` and `自然 CVR（转化率）` stay editable without adding another vertical block. In the normal `.import-assist.info` state, hide the `上传顺序` title and show only compact chips; do not apply this to `.import-assist.warn`.
